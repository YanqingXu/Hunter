...
Unity 与 Android 保留独立联调、真机与发布门槛。
...
SRV-010 在既有战斗切片上将权威数据迁入七类 C++ 对象，并与同名小写 Lua 模块配对。
Lua 保留运动、AI、枪械、伤害及终态规则，高频输入、事件和快照由 C++ 直接处理。
...
| 脚本方向 | Luax Runtime + Bind；C++ 数据与生命周期，Lua 玩法规则 |
...
├─ src/                        # common / core / game / net / script / storage，各模块并置头文件与实现
...
2. 每个功能文件返回模块工厂，显式接收依赖并返回函数表；脚本内部允许 table 和函数引用；闭包只缓存身份视图与句柄，不保存可变玩法数据。
...
| 入口 | 数据与行为 |
| --- | --- |
| `init(context_json)` | 读取初始化上下文，获取原生 World 句柄；返回成功或错误 |
| `on_event(event_id, payload_json)` | 处理低频控制；高频 FrameInput 直接进入 C++ 输入接口 |
...
Protobuf 在 C++ 编解码。FrameInput 在 Tick 边界由 C++ 校验及锁存；快照从原生 World 生成；Lua 通过 `net.event` 标量参数暂存战斗事件。
JSON 仅用于初始化、低频控制及开发状态往返；不在高频网络路径构造或解析 JSON。
...
- 持久状态保存 ID 和到期 Tick，不序列化函数、句柄、循环引用或协程栈；运行中的 Lua 视图可持有代际句柄。
- 快照包含 Tick、实体 ID 分配器、输入去重状态、冷却、AI 状态、局内物品和可序列化随机状态。不得假定内建 random userdata 能自动跨 VM 序列化。
- 导出/导入由 C++ 使用明确 JSON schema 完成；先校验独立候选再发布，并使旧实体及组件句柄失效。不复制 Lua 对象图。
...
| SRV-009 | 基础战斗切片 | 登录、重开、移动、射击、AI、伤害与终态行为回归 |
| SRV-010 | 原生对象与脚本配对 | 七类绑定、代际生命周期、Item、原生网络及 v4 状态 |
