...
SRV-009 的基础实现增量落实 `base.md` 前两轮：在 Luax 拆分实体构造、枪械、伤害与快照，
通过实体 ID 和独立配置／出生 ID 管理世界；协议及内部状态为 v3，内容为 v2。
默认地图和数值保持不变，混合配置及生命周期边界通过隔离测试验收。
该增量不扩展物品、装备、技能、撤离、存档、热更新控制或 Android 宿主范围。
