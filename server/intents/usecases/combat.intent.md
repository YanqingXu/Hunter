...
怪物状态为 spawn、patrol、chase、attack、dead。原生创建时按规范 spawn_id 唯一解析
出生记录，校验配置引用、整数出生坐标、体型、非空巡逻区间、障碍与连续支撑后才发布。
巡逻区间必须包含出生点且完整位于地图内；这些边界在状态导入时同样检查。
spawn 仅表示出生位置上的满血、静止、零冷却对象；Lua 创建入口完成初始化并转入 patrol，
导入的合法 spawn 在下一次 AI 更新时完成初始化。离开 spawn 后不能通过属性写入退回。
Lua 在巡逻端点缩短步长并转向；追击离开巡逻区后失去目标时逐步返回，不瞬移。
检测沿用横纵轴距离均不超过 detect_range；追击水平步长不超过与玩家的距离。
近战要求水平距离不超过 attack_range、竖直相交且无遮挡；移动阶段满足近战条件则停止
水平移动，移动后在攻击阶段再次判断，允许追击进入范围的同一 Tick 发起攻击。
attack_ticks 每个活动 Tick 递减一次，到零才可再次攻击，并重置为配置冷却。
死亡优先于行为更新，Lua 在生命值归零时进入 dead；原生拒绝活体进入 dead 或死体恢复
活态，进入 dead 清空冷却和速度，死亡实体不再巡逻、追击或攻击。
