...
public:
    ...
    std::expected<Accepted, Error> open(Str path, Done done);

    // 读取拥有全部物品的永久玩家；超限返回错误而不截断，回调不得抛异常。
    std::expected<Accepted, Error> load_player(u64 player_id, Done done);

    // 事务分配跨启动不复用的对局 ID，只有完成成功时才可使用。
    std::expected<Accepted, Error> alloc_match(Done done);

    // 接收完整结算值；Accepted 不代表已保存，相同请求可安全重试。
    std::expected<Accepted, Error> commit_match(CommitMatch req, Done done);

    // 查询已提交结果；不存在返回 NotFound，不恢复未完成对局。
    std::expected<Accepted, Error> find_match(u64 match_id, Done done);

    // 独立关闭槽不受容量饱和影响；拒绝新任务，完成全部回调后关闭并通知。
    // 重复 stop 返回 Closed 错误；已接受写入不取消，调用方继续驱动 io。
    std::expected<Accepted, Error> stop(Done done);
