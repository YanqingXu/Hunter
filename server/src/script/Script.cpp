...
Expect<Str, Str> read_file(const Str& path, usize limit)
{
    std::ifstream stream(path, std::ios::binary | std::ios::ate);
    const auto size = stream.tellg();
    if (!stream || size < 0 || static_cast<u64>(size) > limit)
    {
        return Unexpect("artifact_read_failed: " + path);
    }
    ...
    if (!stream)
    {
        return Unexpect("artifact_read_incomplete: " + path);
    }
...
template <usize N>
bool read_hex(const nlohmann::json& value, Arr<std::byte, N>& bytes)
{
    if (!value.is_string())
    {
        return false;
...
Expect<luax::ProductionBundlePolicy, Str> read_policy(const Str& path)
{
    auto text = read_file(path, 1024 * 1024);
    if (!text)
    {
        return Unexpect(text.error());
    }
    ...
    {
        return Unexpect("invalid_bundle_policy");
    }
    ...
    {
        return Unexpect("invalid_bundle_public_key");
    }
    ...
    const Arr names{"bundle", "bytecode", "compiler", "runtime", "language",
        "host_api", "capability", "state", "effect"};
    const Arr fields{&compat.bundle, &compat.bytecode, &compat.compiler, &compat.runtime,
        &compat.language, &compat.hostApi, &compat.capabilities, &compat.stateSchema,
        &compat.effectSchema};
    const auto& ids = obj["identities"];
    const Arr expected{schema::host_api_hash, schema::capability_hash,
    ...
    for (usize index = 0; index < expected.size(); ++index)
    {
        ...
        {
            return Unexpect("host_contract_mismatch: " + Str(names[index + 5]));
        }
    }
    ...
    for (usize index = 0; index < names.size(); ++index)
    {
        ...
        {
            return Unexpect("invalid_bundle_identity: " + Str(names[index]));
        }
    }
...
struct Script::Impl
{
    ...
    Expect<void, Str> enter() const
    {
        ...
        if (busy)
        {
            return Unexpect("reentrant_call");
        }
    ...
    luax::Status register_hosts()
    {
        ...
        diagnostics.add(luax::HostNamespaceEntry::function("log", luax::bind::function(
            [this](Str msg) -> luax::Result<bool>
            {
                ...
                {
                    return Unexpect(host_error("diagnostic_capacity"));
                }
    ...
    Expect<Result, Str> invoke(const Str& name, bool output, Args&&... args)
    {
        auto guard = enter();
        if (!guard)
        {
        ...
        if (!result)
        {
            return Unexpect(fail(name + ": " + Str(result.error().message())));
        }

        if (!host_fault.empty())
        {
            return Unexpect(fail(host_fault));
        }
    ...
    Expect<Vec<ScriptOut>, Str> commit(Expect<bool, Str> result)
    {
        if (!result)
        {
        ...
        if (!*result)
        {
            return Unexpect(fail("script_entry_rejected"));
        }
        ...
        for (const auto& out : pending)
        {
            ...
            if (!decoded)
            {
                return Unexpect(fail(decoded.error()));
            }
        }
...
Expect<Vec<ScriptOut>, Str> Script::open(const Cfg& cfg, const Str& ctx_json)
{
    auto& self = *impl_;
    ...
    {
        return Unexpect("invalid_open_state_or_context");
    }
    ...
    {
        return Unexpect(self.fail(error.what()));
    }
    ...
    if (!rt)
    {
        return Unexpect(self.fail(Str(rt.error().message())));
    }
    ...
    if (!isolate)
    {
        return Unexpect(self.fail(Str(isolate.error().message())));
    }

    self.isolate = *isolate;
    luax::Result<luax::ModuleHandle> module = Unexpect(host_error("missing_artifact"));
#if !HUNTER_PRODUCTION
    ...
    if (cfg.bundle_path.empty())
    {
        ...
        if (!source)
        {
            return Unexpect(self.fail(source.error()));
        }
...
#endif
    {
        ...
        if (!bundle)
        {
            return Unexpect(self.fail(bundle.error()));
        }

        module = self.isolate.loadBundle(std::as_bytes(Span(*bundle)), cfg.bundle_path);
    }

    if (!module)
    {
        return Unexpect(self.fail(Str(module.error().message())));
    }
    ...
    if (!hosts)
    {
        return Unexpect(self.fail(Str(hosts.error().message())));
    }
    ...
    if (!top || !*top || !self.host_fault.empty())
    {
        ...
        return Unexpect(self.fail(error));
    }
        ...
        "validate_state", "shutdown"})
    {
        ...
        if (!func)
        {
            return Unexpect(self.fail(Str(func.error().message())));
        }
...
Expect<Vec<ScriptOut>, Str> Script::event(i64 event_id, const Str& payload_json)
{
    ...
    {
        return Unexpect(impl_->fail("invalid_event_json"));
    }
...
Expect<Vec<ScriptOut>, Str> Script::input(const wire::FrameInput& input, u64 applied_tick)
{
    auto& self = *impl_;
    ...
    try
    {
        ...
        {
            return Unexpect(self.fail("output_capacity"));
        }
...
Expect<Vec<ScriptOut>, Str> Script::tick(u64 tick_id, f64 dt_seconds)
{
    auto guard = impl_->enter();
    if (!guard)
    {
    ...
    {
        return Unexpect(impl_->fail("invalid_tick_arguments"));
    }
    ...
    if (game)
    {
        ...
        {
            return Unexpect(self.fail("invalid_game_tick"));
        }
...
Expect<Str, Str> Script::export_state()
{
...
Expect<void, Str> Script::import_state(const Str& snapshot_json)
{
    auto guard = impl_->enter();
    if (!guard)
    {
...
Expect<void, Str> Script::validate_state()
{
...
Expect<void, Str> Script::change(Func<void(World&)> action)
{
    auto& self = *impl_;
    const auto guard = self.enter();
    if (!guard)
...
Expect<ScriptStats, Str> Script::stats() const
{
    const auto guard = impl_->enter();
    if (!guard)
    {
    ...
    if (!value)
    {
        return Unexpect(Str(value.error().message()));
    }
...
Expect<void, Str> Script::shutdown(const Str& reason)
{
    auto& self = *impl_;
    ...
    {
        return Unexpect(self.close_error);
    }
