...
{
    const nlohmann::json ack = {{"v", 5}, {"seq", "18446744073709551615"},
        {"match_id", "18446744073709551615"}, {"applied_tick", "9223372036854775807"}};
    const nlohmann::json snapshot = {{"v", 5}, {"tick_id", "9223372036854775807"},
    ...
    const Vec<hunter::ScriptOut> good{
        ...
        {"login", R"({"v":5,"req_id":"l","player_id":"1","match_id":"0","phase":"Lobby"})"},
        {"start", R"({"v":5,"req_id":"s","match_id":"1","phase":"Playing"})"},
        {"event", R"({"v":5,"match_id":"1","event_id":"2","tick_id":"3","kind":"hit",)"
            R"("actor_id":"1","target_id":"2","x":-100,"y":0,"amount":20})"},
        {"error", R"({"v":5,"code":"stale_input","detail":"","req_id":"","seq":"9",)"
            R"("match_id":"1"})"}};
